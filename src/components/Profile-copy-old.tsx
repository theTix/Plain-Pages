...
const Profile = () => {
    ...
    const handleImgSave = async () => {
        if(userUid) {
            try {
                ...
                await updateImageUrl(userUid, fileName, downloadURL);
    ...
    const updateImageUrl = async(userId:any, fileName: any, downloadURL: any) => {
        try{
            const docRef = doc(db, "users", userId);
            await setDoc(docRef, {
    ...
    <div className="profile-container-background">
        <div className="profile-container">
            <div className="profile-img">
                <img src={newImageFile ? imageUrl : blankImg} alt="" />
            </div>
            ...
            {/* <label className="upload-label" htmlFor="upload-input" onClick={handleImgSave} >
                <input id="upload-input" name="upload-input" type="file" onChange={(e) => setProfileImg(e.target.files ? e.target.files[0] : null)} />
                {!isUploading ? (
                    <button>
                        <TbPencil />
                        <span>Edit</span>
